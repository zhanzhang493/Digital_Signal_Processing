...
if __name__ == '__main__':
    ...
    IDEAL_PHASE_Iterative = np.zeros(len(IDEAL_PHASE_EST))
    G = 0.5 * ALPHA * TS**2 - 0.5 * B * TS
    DELTA = 0.5 * ALPHA * TS**2
    for k in range(1, len(IDEAL_PHASE_EST)):
        IDEAL_PHASE_Iterative[k] = IDEAL_PHASE_Iterative[k-1] + G
        G = G + 2 * DELTA

    IDEAL_PHASE_Iterative = PhaseModule.PhaseModule.phase_wrapping(IDEAL_PHASE_Iterative)
